...
const generateAccessToken = (user) => {
    const payload = {email: user.email, user_name: user.user_name, id: user.id};
...
module.exports = {
  ...
  authenticateUser(userCredentials, callback)
  {
      ...
      checkUserCredentials(userCredentials, users_collection, (err, user) => {
        if(err) {
          ...
          const refreshToken = generateRefreshToken(user);
          users_collection.findOneAndUpdate({email: user.email}, {$set: {refreshToken: refreshToken}}, (err, result) => {
            if(err)
              callback(err)
            else {
  ...
  {
    const users_collection = db.getUserCollection();
    findRefreshToken(token, users_collection, (err, user_id) => {
      if(err) {
        callback(err);
      }
      else {
